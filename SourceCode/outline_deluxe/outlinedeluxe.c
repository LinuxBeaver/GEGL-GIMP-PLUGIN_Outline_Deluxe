...
enum_start (outline_deluxe_list)
   ...
   enum_value (METAL_OUTLINE,  "metal",  N_("Metal Outline"))
   enum_value (ANYTHING_OUTLINE,  "anything",  N_("Anything Outline"))
...
ui_meta ("visible", "!mode {ocoutline, ocbevel, anything,  }" )
...
property_double (solar1, _("Solarization of Red Channel"), 2.7)
    description (_("Alien map filter adjusting Red Channel"))
  value_range (2, 2.8)
ui_meta ("visible", "mode {metal  }" )

property_double  (solar2, _("Solarization of Green Channel"), 2.8)
  value_range (2.2, 2.8)
    description (_("Alien map filter adjusting Green Channel"))
ui_meta ("visible", "mode {metal  }" )


property_double  (solar3, _("Solarization of Blue Channel"), 2.1)
    description (_("Alien map filter adjusting Blue Channel"))
  value_range (0.0, 4.0)
ui_meta ("visible", "mode {metal  }" )
...
property_string (graph, _("Insert GEGL syntax"), CUSTOM_INPUT)
    ui_meta ("multiline", "false")
    description (_("Input GEGL syntax to make the outline do anything within the limits of GEGL, try chaining other filters"))
ui_meta ("visible", "mode {anything  }" )
...
static void attach (GeglOperation *operation)
{
 ...
 state->chrome   = gegl_node_new_child (gegl,
                                  "operation", "lb:chrome", "bvradius", 6.5, "th", 0.25, 
                                  NULL);
...
/*Fun fact. DST is the invisible blend mode in GEGL. It contains effects like they are in a hidden "layer group" and calls them later with ref from the (id ref) system*/
 ...
 state->median   = gegl_node_new_child (gegl,
  ...
  gegl_operation_meta_redirect (operation, "solar1",  state->chrome, "solar1");
  gegl_operation_meta_redirect (operation, "solar2",  state->chrome, "solar2");
  gegl_operation_meta_redirect (operation, "solar3",  state->chrome, "solar3");
  gegl_operation_meta_redirect (operation, "color",  state->color, "value");
  gegl_operation_meta_redirect (operation, "graph",  state->graph, "string");
...
switch (o->mode) {
 ...
 gegl_node_connect ( state->dst, "aux",  state->idref, "output");
  gegl_node_link_many ( state->idref, state->median, state->idref2, NULL);
        break;
    case METAL_OUTLINE:
  gegl_node_link_many ( state->input, state->ssg,  state->chrome, state->idref3, state->lchcolor, state->crop, state->move,  state->output, NULL);
 gegl_node_connect ( state->lchcolor, "aux",  state->color, "output");
 gegl_node_connect ( state->crop, "aux",  state->idref3, "output");
        break;
    case ANYTHING_OUTLINE:
  gegl_node_link_many ( state->input, state->ssg, state->graph, state->output, NULL);
...
GeglOperationMetaClass *operation_meta_class = GEGL_OPERATION_META_CLASS (klass);
  ...
  gegl_operation_class_set_keys (operation_class,
    ...
    "description", _("A outline filter with many options. Do a normal outline, a beveled outline, outline by original color, metal outline, or custom syntax outline which makes anything possible"
                     ""),
